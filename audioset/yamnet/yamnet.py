...
def yamnet_model(params):
    ...
    predictions, embeddings = yamnet(inputs, params)
    model = Model(name='YAMNet', inputs=inputs, outputs=predictions)
    return model
...
def yamnet_frames_model(params):
  ...
  if params.tflite_ouput == 2:
    predictions, predictions_1, embeddings = yamnet(features, params)
    frames_model = Model(
        name='yamnet_frames', inputs=waveform,
        outputs=[predictions, predictions_1, embeddings, log_mel_spectrogram])
  else:
