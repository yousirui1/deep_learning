...
Reference:
  - [EfficientNet: Rethinking Model Scaling for Convolutional Neural Networks](
      ...
      ~~ ADAPTED BY MAZIN TO INCLUDE BASIC ATTENTION UNIT
"""
## to do stem bn_axis 3 -> 1 
...
def upsample_block(index, filters: int, size: int):
    ''' Upsample block '''
    initializer = CONV_KERNEL_INITIALIZER

    block = Sequential()
    block.add(layers.Conv2DTranspose(filters, size, strides=2, padding='same',
                kernel_initializer=initializer, use_bias=False, name='upsample_conv_' + str(index)))
    block.add(layers.BatchNormalization(name='upsample_bn_' + str(index)))
    block.add(layers.ReLU(name='upsample_relu_' + str(index)))

    return block

def desample_block(index, filters: int, size: int):
    ''' Desample block '''
    initializer = CONV_KERNEL_INITIALIZER

    block = Sequential()
    block.add(layers.Conv2D(filters//2, size, strides=1, padding='same',
                kernel_initializer=initializer, use_bias=False, name='Desample_conv_' + str(index)))
    block.add(layers.BatchNormalization(name='Desample_bn' + str(index)))
    block.add(layers.ReLU(name='Desample_relu'))
    block.add(layers.Conv2D(filters, size, strides=2, padding='same',
                kernel_initializer=initializer, use_bias=False, name='Desample_conv2_' + str(index)))
    block.add(layers.BatchNormalization(name='Desample_bn2_' + str(index)))
    block.add(layers.ReLU(name='Desample_relu2_' + str(index)))
...
def EfficientNet(
  ...
  #--------------------------------------------------------------**** CUSTOM AUTO ENCODER ATTENTION UNIT BY US
  # First we build an basic auto-encoder with 5 downsample stages and use skip connections to 4 upsample stages
  auto_encoder_input = tf.keras.layers.Input(shape=input_shape)
  out = auto_encoder_input
  output_tensors = []
  encoder_outpus = []

  up_stack = [
      upsample_block(0, 672, 3),
      upsample_block(1, 240, 3),
      upsample_block(2, 144, 3),
      upsample_block(3, 96, 3),
  ]

  down_stack = [
      desample_block(0, 320, 3),#8x8 bottleneck (Middle one)
      desample_block(1, 672, 3),
      desample_block(2, 240, 3),
      desample_block(3, 144, 3),
      desample_block(4, 96, 3) # First encode block
  ]
  ...
  for down in down_stack:
      out = down(out)
      encoder_outpus.append(out)
  out = layers.Dropout(0.2)(out)
  encoder_outpus[-1] = out

  encoder_stage = Model(inputs=auto_encoder_input, outputs=encoder_outpus, name='auto-encoder-encode')

  encoder_outpus = [] # Reset encoder outputs

  features = encoder_stage(out_norm) # Connect the auto-encoder to the input of EfficientNet base
  out = features[-1] # Get output of encoder 
  skip_connections = reversed(features[:-1]) # Formulate skip connections 

  for up, skip in zip(up_stack, skip_connections): # Joing downsample to upsample blocks
      out = up(out)
      output_tensors.append(out)
      concat = layers.Concatenate()
      out = concat([out, skip])

  output_tensors.reverse() # Output tensors from the upsample stage of auto encoder which we pass to the build stage of EfficientNet
...
#--------------------------------------------------------------
  ...
  for (i, args) in enumerate(blocks_args):
    ...
    for j in range(round_repeats(args.pop('repeats'))):
      ...
      if j == 0:
        # *************INSERT OUR CUSTOM ATTENTION UNIT************
        x = block(
              x,
              activation,
              drop_connect_rate * b / blocks,
          name='block{}{}_'.format(i + 1, chr(j + 97)), count=(i+1), output_tensors=output_tensors,
          **args)
        continue
...
def block(inputs,
          ...
          expand_ratio=1,
          se_ratio=0.,
          id_skip=True, count=None, output_tensors=None):
